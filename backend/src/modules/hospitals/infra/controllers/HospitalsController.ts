import { LastHospitalsHiredService } from "@modules/hospitals/services/LastHospitalsHiredService";
...
export class HospitalsController {
  ...
  async lastHiredByDoctor(req: Request, res: Response) {
    const { id } = req.user;
    const lastHospitalsHired = container.resolve(LastHospitalsHiredService);
    const hospitals = await lastHospitalsHired.execute(id);
    res.status(200).json(hospitals);
  }
}
